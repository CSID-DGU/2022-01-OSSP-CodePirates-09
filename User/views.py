...
def signin(request):
    if request.method == 'POST':  # 메소드가 post로 넘어온 경우
        ...
        data = { # data =JsonParser()가 오류가 뜸. quertdict으로 넘어오는데 json으로 변환이 안된다고 함, 그래서 넘어온 값을 하나하나 맵핑 ?
            "userId" : request.POST['userId'],
            "userPassword" : request.POST['userPassword']
        }
        # data = JSONParser().parse(request.POST)  # 로그인 화면에서 얻어온 값을 json형태로 저장
        ...
        if UserInfo.objects.filter(userId=data.get('userId')).exists():  # 아이디가 같은 것이 있는지 확인
            ...
            if bcrypt.checkpw(data.get('userPassword').encode('UTF-8'), login_user.userPassword.encode('UTF-8')):
                ...
                return render(request, 'User/signup.html')  # 성공시 render를 통해 메인화면으로 이동
            # render와 redirect의 차이점, render는 템플릿을 불러오는 역할, redirect는 url, 즉 다른 view를 불러오는 역할
...
def crawling(request):
    ...
    location = "충무로역" # 나중에는 입력받은 주소로 치환하기 request.get('location')
    category = "밥집" # 나중에는 입력받은 카테고리로 치환하기 request.get('categoey')로 변경
    ...
    search.send_keys('충무로역') # 클릭된 검색창에 특정 단어 입력 //  나중에는 location으로 대체 //
    ...
    if category == "술집":
        driver.find_element_by_xpath('//*[@id="root"]/div/div/div[1]/div[3]/ul/li[1]/div').click()
